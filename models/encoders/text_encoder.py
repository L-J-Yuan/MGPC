...
def default_cache_dir() -> str:
    return os.path.join(os.path.abspath(os.getcwd()), "clip_model_cache")
...
class ImageCLIP(nn.Module):
    ...
    def __init__(
        ...
        self.clip_model, self.preprocess = clip.load(
            clip_name, download_root=cache_dir or default_cache_dir()
        )
    ...
    def forward(
        ...
        if self.ensure_used_params:
            return self._static_multimodal_embed(
                device=device, images=image_seq, texts=text_seq, embeddings=embedding_seq
            )

        result = torch.zeros((batch_size, self.feature_dim), device=device)
        index_images = []
        index_texts = []
        ...
        if len(index_images):
            ...
            for (i, _), emb in zip(index_images, embs):
                result[i] = emb.to(result)
        if len(index_texts):
            embs = self.embed_text((text for _, text in index_texts), device)
    ...
    def _static_multimodal_embed(
        ...
        joined_embs = torch.stack(
            [
                emb.to(device=device, dtype=torch.float32)
                if emb is not None
                else torch.zeros(self.feature_dim, device=device)
                for emb in embeddings
            ],
            dim=0,
        )

        image_flag = torch.tensor([x is not None for x in images], device=device)[
            :, None
        ].expand_as(image_emb)
        text_flag = torch.tensor([x is not None for x in texts], device=device)[
            :, None
        ].expand_as(image_emb)
        emb_flag = torch.tensor(
            [x is not None for x in embeddings], device=device
    ...
    def embed_text(self, prompts: Iterable[str], device) -> torch.Tensor:
        """
        Embed text prompts as an [N x D] tensor.
        """
        enc = self.clip_model.encode_text(
            self._tokenize(list(prompts), truncate=True).to(device)
        ).float()
...
class FrozenImageCLIP(nn.Module):
    ...
    def __init__(self, **kwargs):
        super().__init__()
        self.model = ImageCLIP(dtype=None, ensure_used_params=False, **kwargs)
    ...
    def __call__(
        ...
        text_tokens = self.model(
            batch_size=batch_size, device=device, images=images, texts=texts, embeddings=embeddings
        )
        return text_tokens
